...
export default function NewsCard({ article }) {
    ...
    return <>
        <div className='max-w-md mx-auto bg-white dark:bg-gray-900 hover:scale-105 transition-all rounded-xl shadow-md overflow-hidden hover:shadow-lg duration-300'>
            ...
            <div className='p-4'>
                <Link to={url}>
                    <h2 className='text-xl font-semibold text-gray-800 dark:text-gray-200 hover:text-blue-600 transition-all'>{title}
                    </h2>
                </Link>
                <p className='text-sm text-gray-600 dark:text-gray-300 mt-2'>{description?.length > 100 ? description.slice(0, 100) + "..." : description}
                </p>
